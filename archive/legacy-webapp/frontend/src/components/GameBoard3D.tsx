...
// Direction vectors for piece indicators
...
// Piece direction patterns
const PIECE_DIRECTIONS: Record<string, string[]> = {
  Commander: ['N'],
  Lance: ['N', 'S'],
  Probe: ['N', 'NE', 'NW'],
  Shield: ['N', 'E', 'W'],
...
function GamePiece({ piece, row, col, isSelected, onClick }: GamePieceProps) {
  ...
  const pieceDirections = PIECE_DIRECTIONS[piece.type] || ['N']

  // Calculate actual directions based on piece orientation
  const getActualDirection = (pieceDir: string) => {
    ...
    const currentIdx = dirIndices[piece.orientation || 'N'] || 0
    const pieceIdx = dirIndices[pieceDir] || 0
    const actualIdx = (currentIdx + pieceIdx) % 8
    return directions[actualIdx]
  }

  const actualDirections = pieceDirections.map((dir) => getActualDirection(dir))
  const directionAngles = actualDirections.map((dir) => {
  ...
  return (
    <group position={position}>
      ...
      {directionAngles.map((angle, index) => (
        <mesh
          key={index}
          position={[Math.sin(angle) * 0.4, pieceHeight / 2 + 0.05, Math.cos(angle) * 0.4]}
          rotation={[0, angle, 0]}
        >
      ...
      {/* Piece type label */}
      <Text
        position={[0, pieceHeight / 2 + 0.4, 0]}
        fontSize={0.15}
        color="white"
        anchorX="center"
        anchorY="middle"
