...
class GameState:
        ...
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                ...
                if piece:
                    ...
                    piece["orientation"] = self.get_piece_orientation(
                        piece["orientation"], piece["position"]
                    )
    ...
    def get_piece_orientation(self, base_orientation, position):
        ...
        if (self.is_bottom_on_top and position == "bottom") or (
            not self.is_bottom_on_top and position == "top"
        ):
    ...
    def move_piece(self, from_row, from_col, to_row, to_col):
        ...
        self.log_debug(
            f"Moved {piece['type']} from ({from_row}, {from_col}) to ({to_row}, {to_col})."
        )
        self.end_turn()
        return True
    ...
    def get_valid_moves(self, row, col):
        ...
        piece_directions = PIECE_DIRECTIONS.get(piece_type, [])
        max_steps = PIECE_TYPES.get(piece_type, 0)

        valid_moves = []

        # Get all valid directions this piece can move
        for piece_dir in piece_directions:
            # Calculate actual direction based on piece's current orientation
            actual_direction = self._get_actual_direction(
                piece_dir, piece["orientation"]
            )

            if actual_direction in DIRECTION_VECTORS:
                direction = DIRECTION_VECTORS[actual_direction]
                for step in range(1, max_steps + 1):
                    ...
                    if 0 <= new_row < BOARD_SIZE and 0 <= new_col < BOARD_SIZE:
                        if self.board[new_row][new_col] is None:
                            ...
                            self.board[new_row][new_col]["position"]
                            != piece["position"]
                        ):
                            valid_moves.append((new_row, new_col))
                            break
    ...
    def _get_actual_direction(self, piece_dir, current_orientation):
        """Calculate the actual direction based on piece orientation."""
        # Direction indices for calculation
        dir_indices = {
            "N": 0,
            "NE": 1,
            "E": 2,
        ...
        current_idx = dir_indices.get(current_orientation, 0)
        piece_idx = dir_indices.get(piece_dir, 0)

        # Calculate actual direction based on piece's current orientation
        actual_idx = (current_idx + piece_idx) % 8
        directions = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"]
        return directions[actual_idx]

    def end_turn(self):
        if self.game_mode == "2player":
            self.current_player_position = (
                "top" if self.current_player_position == "bottom" else "bottom"
            )
        else:
    ...
    def _place_side_player_pieces(self):
        ...
        side_positions = {"left": ("E", 4, 0), "right": ("W", 4, 8)}
        for position, (direction, base_row, base_col) in side_positions.items():
            ...
            for i in range(2):
                self.place_piece(
                    base_row + i * 2 - 1, base_col, position, "Lance", direction
                )
                self.place_piece(
                    base_row + i * 2 - 1,
                    base_col + probe_col_offset * 2,
                    position,
                    "Shield",
                    direction,
                )
    ...
    def check_game_over(self):
        ...
        for position, player in self.players.items():
            if player.captured_pieces and all(
                p["type"] != "Commander" for p in player.captured_pieces
            ):
    ...
    def get_captured_pieces(self):
        """Get all captured pieces organized by player."""
        return {
            position: player.captured_pieces
            for position, player in self.players.items()
        }
